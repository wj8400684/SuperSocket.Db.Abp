...
public sealed class PacketAwaitable<TPacket> : IPacketAwaitable where TPacket : MyPackage
{
    private readonly TaskCompletionSource<MyPackage> _promise = new(TaskCreationOptions.RunContinuationsAsynchronously);
    ...
    public void Complete(MyPackage packet)
    {
        if (packet == null) 
